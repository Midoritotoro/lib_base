...
#define __BASE_CONVERTER_SIMD_SET1_FROM_CHAR8_TYPE(bits)														\
	 else if constexpr (std::is_same_v<_ToChar_, char8_t>) { return PP_CAT(PP_CAT(base_constexpr_mm, bits), _set1_epu8(character));}
#else 
#  define __BASE_CONVERTER_SIMD_SET1_FROM_CHAR8_TYPE(bits)
#endif		
...
#  define __BASE_CONVERTER_SIMD_SET1_FROM_CHAR_TYPE(bits)																															\
	if constexpr (std::is_same_v<_ToChar_, char>) {			 return PP_CAT(PP_CAT(base_constexpr_mm, bits), _set1_epi8(character));}		\
	else if constexpr (std::is_same_v<_ToChar_, char16_t>) { return PP_CAT(PP_CAT(base_constexpr_mm, bits), _set1_epu16(character));}	\
	else if constexpr (std::is_same_v<_ToChar_, char32_t>) { return PP_CAT(PP_CAT(base_constexpr_mm, bits), _set1_epu32(character));}	\
	else if constexpr (std::is_same_v<_ToChar_, wchar_t>) {																															\
		if constexpr (sizeof(wchar_t) == 2) { return PP_CAT(PP_CAT(base_constexpr_mm, bits), _set1_epu16(character));}					\
		else if constexpr (sizeof(wchar_t) == 4) { return PP_CAT(PP_CAT(base_constexpr_mm, bits), _set1_epu32(character));}				\
	}																																												\
...
#define __BASE_CONVERTER_NARROWING_CONVERSION_LIMIT_VECTOR_FOR_CHAR8_TYPE(bits)														\
	 else if constexpr (std::is_same_v<_FromChar_, char8_t>) { return PP_CAT(PP_CAT(base_constexpr_mm, bits), _set1_epu8(MaximumIntegralLimit<char8_t>()));}
#else 
#  define __BASE_CONVERTER_NARROWING_CONVERSION_LIMIT_VECTOR_FOR_CHAR8_TYPE(bits)
#endif		
...
#  define __BASE_CONVERTER_NARROWING_CONVERSION_LIMIT_VECTOR_FOR_CHAR_TYPE(bits)																				\
	if constexpr (std::is_same_v<_FromChar_, char>) {			 return PP_CAT(PP_CAT(base_constexpr_mm, bits), _set1_epi8(MaximumIntegralLimit<char>()));}		\
	else if constexpr (std::is_same_v<_FromChar_, char16_t>) { return PP_CAT(PP_CAT(base_constexpr_mm, bits), _set1_epu16(MaximumIntegralLimit<char16_t>()));}	\
	else if constexpr (std::is_same_v<_FromChar_, char32_t>) { return PP_CAT(PP_CAT(base_constexpr_mm, bits), _set1_epu32(MaximumIntegralLimit<char32_t>()));}	\
	else if constexpr (std::is_same_v<_FromChar_, wchar_t>) {																									\
		if constexpr (sizeof(wchar_t) == 2) { return PP_CAT(PP_CAT(base_constexpr_mm, bits), _set1_epu16(MaximumIntegralLimit<wchar_t>()));}					\
		else if constexpr (sizeof(wchar_t) == 4) { return PP_CAT(PP_CAT(base_constexpr_mm, bits), _set1_epu32(MaximumIntegralLimit<wchar_t>()));}				\
	}																																							\
