...
Renderer::Renderer(const String& t)
	: _t(&t)
{}
...
void Renderer::draw(QPainter& p, const PaintContext& context) {
	...
	_palette = context.palette ? context.palette : &style::defaultTextPalette;
		...
		if (const auto color = block->colorIndex()) {
			if (color == 1) {
				if (_quote && _quote->blockquote && _quoteBlockquoteCache) {
					...
					_currentPen = new QPen(_palette->linkFg);
					_currentPenSelected = new QPen(_palette->selectLinkFg);
				}
			}
			...
			_currentPen = new QPen(_palette->monoFg);
			_currentPenSelected = new QPen(_palette->selectMonoFg);
		}
