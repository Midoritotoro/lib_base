...
namespace internal {
	...
	namespace {
		...
		std::map<uint64, uint32> QtFontsKeys;
		  ...
		  uint32 FontKey(int size, FontFlags flags, int family) {
			return (uint32(family) << 18)
				| (uint32(size) << 6)
				| uint32(flags.value());
		}

		  uint64 QtFontKey(const QFont& font) {
		...
		struct Metrics {
		  ...
		  Metrics ComputeMetrics(QFont font, bool adjust) {
