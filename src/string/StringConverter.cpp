...
static_assert(std::is_nothrow_move_constructible_v<StringEncoder>);
static_assert(std::is_nothrow_move_assignable_v<StringEncoder>);
static_assert(std::is_nothrow_move_constructible_v<StringDecoder>);
static_assert(std::is_nothrow_move_assignable_v<StringDecoder>);
...
static always_inline uint __BitScanReverse(unsigned v) noexcept
{
...
static inline const uchar *simdFindNonAscii(const uchar *src, const uchar *end, const uchar *&nextAscii)
{
    ...
    for ( ; end - src >= 32; src += 32) {
        ...
        return src + CountTrailingZeroBits(n);
    }
#endif
    ...
    for ( ; end - src >= 4; src += 4) {
        quint32 data = FromUnaligned<quint32>(src);
        data &= 0x80808080U;
        if (!data)
            continue;
...
{
    int bitSpacing = 1;
    qptrdiff len = std::min(end8 - src8, end16 - src16);
    qptrdiff offset = 0;
    uint mask = 0;
    ...
    if (mask)
        offset += CountTrailingZeroBits(mask) >> bitSpacing;
    src8 += offset;
    src16 += offset;
}
...
static inline bool simdEncodeAscii(uchar *&dst, const char16_t *&nextAscii, const char16_t *&src, const char16_t *end)
{
    ...
    for ( ; end - src >= 16; src += 16, dst += 16) {
        ...
        if (nonAscii) {
            ...
            nonAscii = CountTrailingZeroBits(nonAscii);
            dst += nonAscii;
            src += nonAscii;
            return false;
        }
...
io::ByteArray Utf8::convertFromUnicode(StringView in)
{
    qsizetype len = in.size();

    // create a io::ByteArray with the worst case scenario size
    io::ByteArray result(len * 3, Qt::Uninitialized);
    ...
    while (src != end) {
        ...
        do {
            char16_t u = *src++;
            int res = toUtf8<Utf8BaseTraits>(u, dst, src, end);
            if (res < 0) {
...
io::ByteArray Utf8::convertFromUnicode(StringView in, StringConverterBase::State *state)
{
    io::ByteArray ba(3*in.size() +3, Qt::Uninitialized);
    char *end = convertFromUnicode(ba.data(), in, state);
    ba.truncate(end - ba.data());
    return ba;
}

char *Utf8::convertFromUnicode(char *out, StringView in, StringConverter::State *state)
{
    Q_ASSERT(state);
    qsizetype len = in.size();
    if (!len)
    ...
    auto appendReplacementChar = [state](uchar *cursor) -> uchar * {
        if (state->flags & StringConverter::Flag::ConvertInvalidToNull) {
            *cursor++ = 0;
        } else {
            // Char::replacement encoded in utf8
            *cursor++ = 0xef;
            *cursor++ = 0xbf;
            *cursor++ = 0xbd;
        }
    ...
    if (!(state->flags & StringDecoder::Flag::Stateless)) {
        if (state->remainingChars) {
            int res = Utf8Functions::toUtf8<Utf8BaseTraits>(state->state_data[0], cursor, src, end);
            if (res < 0)
                cursor = appendReplacementChar(cursor);
            state->state_data[0] = 0;
            state->remainingChars = 0;
        } else if (!(state->internalState & HeaderDone) && state->flags & StringConverter::Flag::WriteBom) {
            // append UTF-8 BOM
            *cursor++ = utf8bom[0];
            *cursor++ = utf8bom[1];
            *cursor++ = utf8bom[2];
    ...
    while (src != end) {
        ...
        do {
            char16_t uc = *src++;
            int res = Utf8Functions::toUtf8<Utf8BaseTraits>(uc, cursor, src, end);
            if (Q_LIKELY(res >= 0))
                continue;
            ...
            if (res == Utf8BaseTraits::Error) {
                ...
                if (state->flags & StringConverter::Flag::Stateless) {
                    ++state->invalidChars;
                    cursor = appendReplacementChar(cursor);
                } else {
...
char *Utf8::convertFromLatin1(char *out, Latin1StringView in)
{
    // ### SIMD-optimize:
    for (uchar ch : in) {
        if (ch < 128) {
...
QString Utf8::convertToUnicode(io::ByteArrayView in)
{
    ...
    Char *data = const_cast<Char*>(result.constData()); // we know we're not shared
    const Char *end = convertToUnicode(data, in);
...
/*! \internal
    ...
    Char starting at \a dst in the destination buffer. The buffer is expected
    to be large enough to hold the result. An upper bound for the size of the
    buffer is \c in.size() Chars.

    If, during decoding, an error occurs, a Char::ReplacementCharacter is
    written.

    Returns a pointer to one past the last Char written.

    This function never throws.

    For Char buffers, instead of casting manually, you can use the static
    Utf8::convertToUnicode(Char *, io::ByteArrayView) directly.
*/
char16_t *Utf8::convertToUnicode(char16_t *dst, io::ByteArrayView in) noexcept
{
    ...
    if (!simdDecodeAscii(dst, nextAscii, src, end)) {
        ...
        while (src < end) {
            ...
            do {
                uchar b = *src++;
                const qsizetype res = Utf8Functions::fromUtf8<Utf8BaseTraits>(b, dst, src, end);
                if (res < 0) {
                    // decoding error
                    *dst++ = Char::ReplacementCharacter;
                }
            } while (src < nextAscii);
        }
    }
...
QString Utf8::convertToUnicode(io::ByteArrayView in, StringConverter::State *state)
{
    ...
    Char *end = convertToUnicode(result.data(), in, state);
    result.truncate(end - result.constData());
    return result;
}

char16_t *Utf8::convertToUnicode(char16_t *dst, io::ByteArrayView in, StringConverter::State *state)
{
    qsizetype len = in.size();
    ...
    char16_t replacement = Char::ReplacementCharacter;
    if (state->flags & StringConverter::Flag::ConvertInvalidToNull)
    ...
    if (!(state->flags & StringConverter::Flag::Stateless)) {
        bool headerdone = state->internalState & HeaderDone || state->flags & StringConverter::Flag::ConvertInitialBom;
    ...
    } else if (!(state->flags & StringConverter::Flag::ConvertInitialBom)) {
...
Utf8::ValidUtf8Result Utf8::isValidUtf8(io::ByteArrayView in)
{
    const uchar *src = reinterpret_cast<const uchar *>(in.data());
    const uchar *end = src + in.size();
    const uchar *nextAscii = src;
    ...
    while (src < end) {
        ...
        do {
            ...
            isValidAscii = false;
            Utf8NoOutputTraits::NoOutput output;
            const qsizetype res = Utf8Functions::fromUtf8<Utf8NoOutputTraits>(b, output, src, end);
            if (res < 0) {
                // decoding error
                return { false, false };
            }
...
int Utf8::compareUtf8(io::ByteArrayView utf8, StringView utf16, Qt::CaseSensitivity cs) noexcept
{
    ...
    do {
        ...
        if (src1 < end1 && src2 < end2) {
            ...
            if (uc1 >= 0x80) {
                char32_t *output = &uc1;
                qsizetype res = Utf8Functions::fromUtf8<Utf8BaseTraitsNoAscii>(uc1, output, src1, end1);
                if (res < 0) {
                    // decoding error
                    uc1 = Char::ReplacementCharacter;
                }

                // Only decode the UTF-16 surrogate pair if the UTF-8 code point
                // wasn't US-ASCII (a surrogate cannot match US-ASCII).
                if (Char::isHighSurrogate(uc2) && src2 < end2 && Char::isLowSurrogate(*src2))
                    uc2 = Char::surrogateToUcs4(uc2, *src2++);
            }
            if (cs == CaseInsensitive) {
                uc1 = Char::toCaseFolded(uc1);
                uc2 = Char::toCaseFolded(uc2);
            }
            if (uc1 != uc2)
...
int Utf8::compareUtf8(io::ByteArrayView utf8, Latin1StringView s, Qt::CaseSensitivity cs)
{
    char32_t uc1 = Char::Null;
    ...
    while (src1 < end1 && src2 < end2) {
        uchar b = *src1++;
        char32_t *output = &uc1;
        const qsizetype res = fromUtf8<Utf8BaseTraits>(b, output, src1, end1);
        if (res < 0) {
            // decoding error
            uc1 = Char::ReplacementCharacter;
        }
...
int Utf8::compareUtf8(io::ByteArrayView lhs, io::ByteArrayView rhs, Qt::CaseSensitivity cs) noexcept
{
    if (lhs.isEmpty())
    ...
    while (src1 < end1 && src2 < end2) {
        uchar b = *src1++;
        char32_t *output = &uc1;
        qsizetype res = Utf8Functions::fromUtf8<Utf8BaseTraits>(b, output, src1, end1);
        if (res < 0) {
            // decoding error
            uc1 = Char::ReplacementCharacter;
        }
        ...
        b = *src2++;
        output = &uc2;
        res = Utf8Functions::fromUtf8<Utf8BaseTraits>(b, output, src2, end2);
        if (res < 0) {
            // decoding error
            uc2 = Char::ReplacementCharacter;
        }
...
io::ByteArray Utf16::convertFromUnicode(StringView in, StringConverter::State *state, DataEndianness endian)
{
    bool writeBom = !(state->internalState & HeaderDone) && state->flags & StringConverter::Flag::WriteBom;
    qsizetype length = 2 * in.size();
    if (writeBom)
        length += 2;

    io::ByteArray d(length, Qt::Uninitialized);
...
char *Utf16::convertFromUnicode(char *out, StringView in, StringConverter::State *state, DataEndianness endian)
{
    Q_ASSERT(state);
    bool writeBom = !(state->internalState & HeaderDone) && state->flags & StringConverter::Flag::WriteBom;
...
QString Utf16::convertToUnicode(io::ByteArrayView in, StringConverter::State *state, DataEndianness endian)
{
    QString result((in.size() + 1) >> 1, Qt::Uninitialized); // worst case
    Char *qch = convertToUnicode(result.data(), in, state, endian);
    result.truncate(qch - result.constData());
    return result;
}

Char *Utf16::convertToUnicode(Char *out, io::ByteArrayView in, StringConverter::State *state, DataEndianness endian)
{
    qsizetype len = in.size();
    ...
    if (state->flags & StringConverter::Flag::ConvertInitialBom)
        headerdone = true;
    ...
    if (!headerdone || state->remainingChars) {
        ...
        if (endian == DetectEndianness) {
            ...
            if (ch == Char::ByteOrderSwapped) {
                endian = BigEndianness;
            } else if (ch == Char::ByteOrderMark) {
        ...
        if (endian == BigEndianness)
            ch = Char::fromUcs2((ch.unicode() >> 8) | ((ch.unicode() & 0xff) << 8));
        if (headerdone || ch != Char::ByteOrderMark)
            *out++ = ch;
    ...
    if ((end - chars) & 1) {
        if (state->flags & StringConverter::Flag::Stateless) {
            *out++ = state->flags & StringConverter::Flag::ConvertInvalidToNull ? Char::Null : Char::ReplacementCharacter;
        } else {
            state->remainingChars = 1;
...
io::ByteArray QUtf32::convertFromUnicode(StringView in, StringConverter::State *state, DataEndianness endian)
{
    bool writeBom = !(state->internalState & HeaderDone) && state->flags & StringConverter::Flag::WriteBom;
    qsizetype length =  4*in.size();
    if (writeBom)
        length += 4;
    io::ByteArray ba(length, Qt::Uninitialized);
    char *end = convertFromUnicode(ba.data(), in, state, endian);
    ba.truncate(end - ba.constData());
    return ba;
}

char *QUtf32::convertFromUnicode(char *out, StringView in, StringConverter::State *state, DataEndianness endian)
{
    Q_ASSERT(state);
...
decode_surrogate:
            if (uc == end) {
                if (state->flags & StringConverter::Flag::Stateless) {
                    ucs4 = state->flags & StringConverter::Flag::ConvertInvalidToNull ? 0 : Char::ReplacementCharacter;
                } else {
                    state->remainingChars = 1;
                ...
                ucs4 = Char::surrogateToUcs4(ch, *uc++);
            } else {
                ucs4 = state->flags & StringConverter::Flag::ConvertInvalidToNull ? 0 : Char::ReplacementCharacter;
            }
        } else {
...
QString QUtf32::convertToUnicode(io::ByteArrayView in, StringConverter::State *state, DataEndianness endian)
{
    QString result;
    result.resize((in.size() + 7) >> 1); // worst case
    Char *end = convertToUnicode(result.data(), in, state, endian);
    result.truncate(end - result.constData());
    return result;
}

Char *QUtf32::convertToUnicode(Char *out, io::ByteArrayView in, StringConverter::State *state, DataEndianness endian)
{
    qsizetype len = in.size();
    const char *chars = in.data();
    ...
    if (!headerdone || endian == DetectEndianness || num) {
        ...
        if (headerdone || code != Char::ByteOrderMark) {
            if (Char::requiresSurrogates(code)) {
                *out++ = Char(Char::highSurrogate(code));
                *out++ = Char(Char::lowSurrogate(code));
            } else {
                *out++ = Char(code);
            }
        }
    ...
    while (chars < end) {
        ...
        if (num == 4) {
            char32_t code = (endian == BigEndianness) ? qFromBigEndian<char32_t>(tuple) : qFromLittleEndian<char32_t>(tuple);
            for (char16_t c : Char::fromUcs4(code))
                *out++ = c;
            num = 0;
        }
    }
...
QString QLocal8Bit::convertToUnicode_sys(io::ByteArrayView in, StringConverter::State *state)
{
    return convertToUnicode_sys(in, CP_ACP, state);
}

QString QLocal8Bit::convertToUnicode_sys(io::ByteArrayView in, quint32 codePage,
                                         StringConverter::State *state)
{
    ...
    using Flag = StringConverter::Flag;
    const bool useNullForReplacement = !!(state->flags & Flag::ConvertInvalidToNull);
    const char16_t replacementCharacter = useNullForReplacement ? Char::Null
    ...
    if (state && state->remainingChars) {
        ...
        if (len) {
            state->remainingChars = 0;
            if (mblen == 0)
                return StringView(out, len).toString();
            out += len;
            outlen -= len;
    ...
    if (sp.isEmpty()) {
        ...
        if (out != buf.data()) // else: we return null-string
            sp = StringView(buf.data(), out).toString();
    } else{
...
io::ByteArray QLocal8Bit::convertFromUnicode_sys(StringView in, StringConverter::State *state)
{
    return convertFromUnicode_sys(in, CP_ACP, state);
}

io::ByteArray QLocal8Bit::convertFromUnicode_sys(StringView in, quint32 codePage,
                                              StringConverter::State *state)
{
    ...
    if (state && state->remainingChars > 0) {
        ...
        const bool validCodePoint = Char::isLowSurrogate(wc[1]);
        ...
        state->state_data[0] = 0;
        if (uclen == 0)
            return io::ByteArrayView(buf.data(), len).toByteArray();
    }

    if (state && Char::isHighSurrogate(ch[uclen - 1])) {
    ...
    const auto getNextWindowSize = [&]() {
        ...
        if (nextIn > 1 && Char::isHighSurrogate(ch[nextIn - 1]))
            --nextIn;
        return nextIn;
    };
            ...
            if (r == ERROR_INSUFFICIENT_BUFFER) {
                ...
                fprintf(stderr,
                        ...
                        reinterpret_cast<const wchar_t *>(
                                StringView(ch, uclen).left(100).toString().utf16()));
#endif
                break;
            }
        }
    ...
    if (mb.isEmpty()) {
        ...
        if (out != buf.data()) // else: we return null-array
            mb = io::ByteArrayView(buf.data(), out).toByteArray();
    } else {
...
void StringConverter::State::clear() noexcept
{
    if (clearFn)
        clearFn(this);
    else
...
void StringConverter::State::reset() noexcept
{
...
static Char *fromUtf16(Char *out, io::ByteArrayView in, StringConverter::State *state)
{
    return Utf16::convertToUnicode(out, in, state, DetectEndianness);
}

static char *toUtf16(char *out, StringView in, StringConverter::State *state)
{
    return Utf16::convertFromUnicode(out, in, state, DetectEndianness);
}

static Char *fromUtf16BE(Char *out, io::ByteArrayView in, StringConverter::State *state)
{
    return Utf16::convertToUnicode(out, in, state, BigEndianness);
}

static char *toUtf16BE(char *out, StringView in, StringConverter::State *state)
{
    return Utf16::convertFromUnicode(out, in, state, BigEndianness);
}

static Char *fromUtf16LE(Char *out, io::ByteArrayView in, StringConverter::State *state)
{
    return Utf16::convertToUnicode(out, in, state, LittleEndianness);
}

static char *toUtf16LE(char *out, StringView in, StringConverter::State *state)
{
    return Utf16::convertFromUnicode(out, in, state, LittleEndianness);
}

static Char *fromUtf32(Char *out, io::ByteArrayView in, StringConverter::State *state)
{
    return QUtf32::convertToUnicode(out, in, state, DetectEndianness);
}

static char *toUtf32(char *out, StringView in, StringConverter::State *state)
{
    return QUtf32::convertFromUnicode(out, in, state, DetectEndianness);
}

static Char *fromUtf32BE(Char *out, io::ByteArrayView in, StringConverter::State *state)
{
    return QUtf32::convertToUnicode(out, in, state, BigEndianness);
}

static char *toUtf32BE(char *out, StringView in, StringConverter::State *state)
{
    return QUtf32::convertFromUnicode(out, in, state, BigEndianness);
}

static Char *fromUtf32LE(Char *out, io::ByteArrayView in, StringConverter::State *state)
{
    return QUtf32::convertToUnicode(out, in, state, LittleEndianness);
}

static char *toUtf32LE(char *out, StringView in, StringConverter::State *state)
{
    return QUtf32::convertFromUnicode(out, in, state, LittleEndianness);
}

char *QLatin1::convertFromUnicode(char *out, StringView in, StringConverter::State *state) noexcept
{
    Q_ASSERT(state);
    if (state->flags & StringConverter::Flag::Stateless) // temporary
        state = nullptr;

    const char replacement = (state && state->flags & StringConverter::Flag::ConvertInvalidToNull) ? 0 : '?';
    qsizetype invalid = 0;
...
static Char *fromLocal8Bit(Char *out, io::ByteArrayView in, StringConverter::State *state)
{
    QString s = QLocal8Bit::convertToUnicode(in, state);
    memcpy(out, s.constData(), s.size()*sizeof(Char));
    return out + s.size();
}

static char *toLocal8Bit(char *out, StringView in, StringConverter::State *state)
{
    io::ByteArray s = QLocal8Bit::convertFromUnicode(in, state);
...
const StringConverter::Interface StringConverter::encodingInterfaces[StringConverter::LastEncoding + 1] =
{
    { "UTF-8", Utf8::convertToUnicode, fromUtf8Len, Utf8::convertFromUnicode, toUtf8Len },
...
#if QT_CONFIG(icu)
// only derives from StringConverter to get access to protected types
struct StringConverterICU : StringConverter
{
    static void clear_function(StringConverterBase::State *state) noexcept
    {
        ucnv_close(static_cast<UConverter *>(state->d[0]));
        state->d[0] = nullptr;
    }

    static void ensureConverter(StringConverter::State *state)
    {
    ...
    static Char *toUtf16(Char *out, io::ByteArrayView in, StringConverter::State *state)
    {
        ...
        // If the StringConverter was moved, the state that we used as a context is stale now.
        ...
        if (state->flags.testFlag(StringConverter::Flag::Stateless)) {
    ...
    static char *fromUtf16(char *out, StringView in, StringConverter::State *state)
    {
        ensureConverter(state);
        auto icu_conv = static_cast<UConverter *>(state->d[0]);
        UErrorCode err = U_ZERO_ERROR;
    ...
    Q_DISABLE_COPY_MOVE(StringConverterICU)
    ...
    static constexpr StringConverter::Interface forLength[] = {
        {"icu, recompile if you see this", StringConverterICU::toUtf16, StringConverterICU::toLen, StringConverterICU::fromUtf16, StringConverterICU::fromLen<1>},
        {"icu, recompile if you see this", StringConverterICU::toUtf16, StringConverterICU::toLen, StringConverterICU::fromUtf16, StringConverterICU::fromLen<2>},
        {"icu, recompile if you see this", StringConverterICU::toUtf16, StringConverterICU::toLen, StringConverterICU::fromUtf16, StringConverterICU::fromLen<3>},
        {"icu, recompile if you see this", StringConverterICU::toUtf16, StringConverterICU::toLen, StringConverterICU::fromUtf16, StringConverterICU::fromLen<4>},
        {"icu, recompile if you see this", StringConverterICU::toUtf16, StringConverterICU::toLen, StringConverterICU::fromUtf16, StringConverterICU::fromLen<5>},
        {"icu, recompile if you see this", StringConverterICU::toUtf16, StringConverterICU::toLen, StringConverterICU::fromUtf16, StringConverterICU::fromLen<6>},
        {"icu, recompile if you see this", StringConverterICU::toUtf16, StringConverterICU::toLen, StringConverterICU::fromUtf16, StringConverterICU::fromLen<7>},
        {"icu, recompile if you see this", StringConverterICU::toUtf16, StringConverterICU::toLen, StringConverterICU::fromUtf16, StringConverterICU::fromLen<8>}
    };
    ...
    static const StringConverter::Interface *make_icu_converter(
            StringConverterBase::State *state,
            const char *name)
    {
        UErrorCode status = U_ZERO_ERROR;
        ...
        state->d[0] = conv;
        state->flags |= StringConverterBase::Flag::UsesIcu;
        qsizetype maxCharSize = ucnv_getMaxCharSize(conv);
        state->clearFn = StringConverterICU::clear_function;
...
StringConverter::StringConverter(const char *name, Flags f)
    : iface(nullptr), state(f)
{
    ...
    if (e)
        ...
        iface = StringConverterICU::make_icu_converter(&state, name);
#endif
}


const char *StringConverter::name() const noexcept
{
    if (!iface)
        return nullptr;
    if (state.flags & StringConverter::Flag::UsesIcu) {
#if QT_CONFIG(icu)
...
std::optional<StringConverter::Encoding> StringConverter::encodingForName(const char *name) noexcept
{
    if (!name)
        return std::nullopt;
    for (qsizetype i = 0; i < LastEncoding + 1; ++i) {
        if (nameMatch(encodingInterfaces[i].name, name))
            return StringConverter::Encoding(i);
    }
    if (nameMatch(name, "latin1"))
        return StringConverter::Latin1;
    return std::nullopt;
}
 ...
 */
std::optional<StringConverter::Encoding>
StringConverter::encodingForData(io::ByteArrayView data, char16_t expectedFirstCharacter) noexcept
{
    // someone set us up the BOM?
    qsizetype arraySize = data.size();
    if (arraySize > 3) {
        char32_t uc = qFromUnaligned<char32_t>(data.data());
        if (uc == qToBigEndian(char32_t(Char::ByteOrderMark)))
            return StringConverter::Utf32BE;
        if (uc == qToLittleEndian(char32_t(Char::ByteOrderMark)))
            return StringConverter::Utf32LE;
    ...
    if (arraySize > 2) {
        if (memcmp(data.data(), utf8bom, sizeof(utf8bom)) == 0)
            return StringConverter::Utf8;
    }

    if (arraySize > 1) {
        char16_t uc = qFromUnaligned<char16_t>(data.data());
        if (uc == qToBigEndian(char16_t(Char::ByteOrderMark)))
            return StringConverter::Utf16BE;
        if (uc == qToLittleEndian(char16_t(Char::ByteOrderMark)))
            return StringConverter::Utf16LE;
        if (expectedFirstCharacter) {
            // catch also anything starting with the expected character
            if (qToLittleEndian(uc) == expectedFirstCharacter)
                return StringConverter::Utf16LE;
...
static io::ByteArray parseHtmlMetaForEncoding(io::ByteArrayView data)
{
    ...
    io::ByteArray header = data.first(qMin(data.size(), qsizetype(1024))).toByteArray().toLower();
...
std::optional<StringConverter::Encoding> StringConverter::encodingForHtml(io::ByteArrayView data)
{
    // determine charset
    std::optional<StringConverter::Encoding> encoding = encodingForData(data);
    if (encoding)
        // trust the initial BOM
        return encoding;

    io::ByteArray encodingTag = parseHtmlMetaForEncoding(data);
...
{
#if !QT_CONFIG(icu)
    return StringConverter::Encoding::LastEncoding;
#else
...
StringList StringConverter::availableCodecs()
{
    auto availableCodec = [](sizetype index) -> String
    {
    #if !QT_CONFIG(icu)
        return String::fromLatin1(encodingInterfaces[index].name);
    #else
        if (index == 0) // "Locale", not provided by icu
            return QString::fromLatin1(
                        encodingInterfaces[StringConverter::Encoding::System].name);
    ...
    sizetype codecCount = availableCodecCount();
    StringList result;

    result.reserve(codecCount);
    for (sizetype i = 0; i < codecCount; ++i)
        result.push_back(availableCodec(i));
    return result;
}


StringDecoder StringDecoder::decoderForHtml(io::ByteArrayView data)
{
    // determine charset
    std::optional<StringConverter::Encoding> encoding = encodingForData(data);
    if (encoding)
        // trust the initial BOM
        return StringDecoder(encoding.value());

    io::ByteArray encodingTag = parseHtmlMetaForEncoding(data);
    if (!encodingTag.isEmpty())
        return StringDecoder(encodingTag);

    return StringDecoder(Utf8);
}


const char *StringConverter::nameForEncoding(StringConverter::Encoding e)
{
