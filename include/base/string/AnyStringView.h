...
private:
    ...
    using if_convertible_to = std::enable_if_t<std::conjunction_v<
        ...
        std::is_same<std::remove_cvref_t<T>, AnyStringView::Tag>,
        std::is_same<std::remove_cvref_t<T>, AnyStringView>, // don't make a copy/move ctor
        std::is_pointer<std::decay_t<T>>, // const char*, etc
        std::is_same<std::remove_cvref_t<T>, QByteArray>,
        std::is_same<std::remove_cvref_t<T>, QString>
        >>,
    ...
    static_assert(QtPrivate::IsContainerCompatibleWithQStringView<AnyStringView>::value == false);
    static_assert(QtPrivate::IsContainerCompatibleWithQUtf8StringView<AnyStringView>::value == false);

    template<typename Char>
    static constexpr bool isAsciiOnlyCharsAtCompileTime(Char* str, sizetype sz) noexcept
    {
    ...
    template<typename Char>
    static constexpr std::size_t encodeType(const Char* str, sizetype sz) noexcept
    {
    ...
    {
        if (is_constant_evaluated())
            return sizetype(std::char_traits<Char>::length(str));
...
public:
    ...
    constexpr AnyStringView(const Char* str, sizetype len)
        : m_data{ str }, m_size{ encodeType<Char>(str, len) }
    {
    }

    template <typename Char, if_compatible_char<Char> = true>
    constexpr AnyStringView(const Char* f, const Char* l)
        : AnyStringView(f, l - f) {}

#ifdef Q_QDOC
    template <typename Char, size_t N>
    constexpr AnyStringView(const Char(&array)[N]) noexcept;

    template <typename Char>
    constexpr AnyStringView(const Char* str) noexcept;
#else
    ...
    inline AnyStringView(const QByteArray& str) noexcept; // TODO: Should we have this at all? Remove?
    inline AnyStringView(const QString& str) noexcept;
    inline constexpr AnyStringView(QLatin1StringView str) noexcept;

    template <typename Container, if_compatible_container<Container> = true>
    constexpr Q_ALWAYS_INLINE AnyStringView(const Container& c) noexcept
        : AnyStringView(std::data(c), QtPrivate::lengthHelperContainer(c)) {}

    template <typename Container, if_convertible_to<QString, Container> = true>
    constexpr AnyStringView(Container&& c, QtPrivate::wrapped_t<Container, QString>&& capacity = {})
    ...
    constexpr AnyStringView(Container&& c, QtPrivate::wrapped_t<Container, QByteArray>&& capacity = {})
    ...
    template <typename Char, typename Container = decltype(Char::fromUcs4(U'x')),
        std::enable_if_t<std::is_same_v<Char, char32_t>, bool> = true>
    constexpr AnyStringView(Char c, Container&& capacity = {})
        : AnyStringView(capacity = Char::fromUcs4(c)) {}

    constexpr AnyStringView(QStringView v) noexcept
        : AnyStringView(std::data(v), QtPrivate::lengthHelperContainer(v)) {}

    template <bool UseChar8T>
    constexpr AnyStringView(QBasicUtf8StringView<UseChar8T> v) noexcept
        : AnyStringView(std::data(v), QtPrivate::lengthHelperContainer(v)) {}

    template <typename Char, size_t Size, if_compatible_char<Char> = true>
    [[nodiscard]] constexpr static AnyStringView fromArray(const Char(&string)[Size]) noexcept
    {
...
private:
    ...
    {
        return Assert(isUtf16()), QStringView{ m_data_utf16, size() };
    }
    [[nodiscard]] constexpr q_no_char8_t::QUtf8StringView asUtf8StringView() const
    {
        return Assert(isUtf8()), q_no_char8_t::QUtf8StringView{ m_data_utf8, size() };
    }
