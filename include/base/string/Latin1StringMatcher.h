...
namespace base::string {
    template<class RandomIt1,
        ...
        class boyer_moore_searcher_hashed_needle
    {
    public:
        constexpr boyer_moore_searcher_hashed_needle(RandomIt1 pat_first, RandomIt1 pat_last)
            : m_skiptable{}
        {
    ...
    struct CaseSensitiveLatin1Hash
    {
        constexpr CaseSensitiveLatin1Hash() = default;

        constexpr std::size_t operator()(char c) const noexcept { return std::size_t(uchar(c)); }
    };

    struct CaseInsensitiveLatin1Hash
    {
        constexpr CaseInsensitiveLatin1Hash() = default;
    ...
    class Latin1StringMatcher
    {
    public:
        Latin1StringMatcher() noexcept;
        explicit Latin1StringMatcher(
            Latin1StringView pattern, CaseSensitivity cs = CaseSensitive) noexcept;
        ~Latin1StringMatcher() noexcept;

        void setPattern(Latin1StringView pattern) noexcept;
        Latin1StringView pattern() const noexcept;
        void setCaseSensitivity(CaseSensitivity cs) noexcept;
        CaseSensitivity caseSensitivity() const noexcept;

        sizetype indexIn(Latin1StringView haystack, sizetype from = 0) const noexcept;
