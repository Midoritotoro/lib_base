...
struct AbstractConcatenable
{
protected:
    static void convertFromUtf8(io::ByteArrayView in, Char*& out) noexcept;
    static inline void convertFromAscii(char a, Char*& out) noexcept
    {
        *out++ = Latin1Char(a);
    }
    static void appendLatin1To(Latin1StringView in, Char* out) noexcept;
};

    template <typename T> struct Concatenable;

    template <typename T>
    using ConcatenableEx = Concatenable<remove_cvref_t<T>>;
...
{
    io::ByteArray toLatin1() const { return this->resolved().toLatin1(); }
    io::ByteArray toUtf8() const { return this->resolved().toUtf8(); }
    io::ByteArray toLocal8Bit() const { return this->resolved().toLocal8Bit(); }
};

template <typename A, typename B>
class StringBuilder : public StringBuilderBase<StringBuilder<A, B>,
    typename ConvertToTypeHelper<
