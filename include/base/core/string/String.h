...
class StringView;
  ...
  base_constexpr_cxx20 auto Char::fromUcs4(char32_t c) noexcept
{
	struct R {
		char16_t chars[2];
		//  base_constexpr_cxx20 operator BasicString<char16_t, CharTraits<char16_t>, std::allocator<char16_t>>() const noexcept { return { begin(), end() }; }
		  base_constexpr_cxx20 sizetype size() const noexcept { return chars[1] ? 2 : 1; }
		  base_constexpr_cxx20 const char16_t* begin() const noexcept { return chars; }
		  base_constexpr_cxx20 const char16_t* end() const noexcept { return begin() + size(); }
	};
